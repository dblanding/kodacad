...
class MainWindow(QMainWindow):
    ...
    def saveStepTopAsy(self):
        """Save all parts in self._partDict to step file."""

        # Create XCAF document with an empty top assembly & save as self.doc
        self.createEmptyDoc()

        # Add all parts under '/' as components of top assy in self.doc
        self.addComponents()
    ...
    def createEmptyDoc(self):
        ...
        Unfortunately, this doesn't work so instead, load a step file
        containing a top assy with one component, then re-use the xcaf
        document generated by the step reader."""
        ...
        print(shape_tool.BaseLabel)
        # add empty top assembly (This fails)
        aShape = TopoDS_Shape()
        rootLabel = shape_tool.AddComponent(doc.Main(), aShape, True)
        # check to see how many labels are at root
    ...
    def createEmptyDoc(self):
        """Do a 'mock' load step in order to save the XCAF doc as self.doc"""

        fname = "4167.stp"
        logger.debug("Mock Load file name: %s", fname)
        nextUID = self._currentUID
        stepImporter = stepXD.StepImporter(fname, nextUID)
        self.doc = stepImporter.doc  # <class 'OCC.Core.TDocStd.TDocStd_Document'>

    def addComponent(self):
        """Add active part to top assembly of an existing XCAF doc.

        This is step 2 in a sequence of 3 steps of a test:
        Step 1 is to import a step file with a top assembly, saving the
        XCAF document generated by the import as self.doc.
        Step 2 (This) Insert the current Active Part under the
        top assembly of self.doc.
        Step 3 Save the modififed XCAF document as a new step file.
        """
        labels = TDF_LabelSequence()
        ...
        '''
        # Modify self.doc by removing components of top assembly
        ...
        if topComps.Length():
            logger.debug("Number of Components: %i", topComps.Length())
            for j in range(topComps.Length()):
                cLabel = topComps.Value(j+1)
                #cShape = shape_tool.GetShape(cLabel)
                remsh = shape_tool.RemoveShape(cLabel)
                logger.debug("Component removed: %s", remsh)
        '''
    ...
    def saveStep(self):
        """Export self.doc to STEP file. (Step 3 of test above)"""
